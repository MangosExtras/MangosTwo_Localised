...
UPDATE `mangos_string` SET `content_loc8`='Карта: %u (%s) Зона: %u (%s) Область: %u (%s) Фаза: %u$B$BX: %f Y: %f Z: %f Ориентация: %f$B$Bсетка[%u,%u]ячейка[%u,%u] ИНИнстанса: %u$B$B XЗоны: %f YЗоны: %f$B$BZОснования: %f ZПола: %f имеете данные Высот (Карта: %u КартаВ: %u)' WHERE `entry`=101;
...
UPDATE `mangos_string` SET `content_loc8`='Точка маршрута (Только ГМ может увидеть это)' WHERE `entry`=529;
